...
export async function GET(req: NextRequest) {
  try {
    ...
    if (upcoming) query = query.gte('test_date', new Date().toISOString());
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { course_id, title, test_type, test_date, duration_minutes, location, total_marks, passing_marks, instructions, attachments } = body;

    if (!course_id || !title || !test_date || !duration_minutes) {
    ...
    const { data, error } = await supabase
      .from('tests')
      .insert({
        course_id, title, test_type: test_type || 'quiz',
        test_date, duration_minutes, location,
        total_marks: total_marks || 50, passing_marks: passing_marks || 25,
    ...
    if (enrollments) {
      const notifications = enrollments.map((e: any) => ({
        ...
        type: 'test_reminder' as const,
        title: `New Test: ${title}`,
        message: `Test scheduled for ${new Date(test_date).toLocaleDateString()}`,
        data: { test_id: data.id, course_id }
      }));

...
export default function AdminTestCreatePage() {
  ...
  const [form, setForm] = useState<{ course_id: string; lecturer_id: string; title: string; test_type: string; test_date: string; duration_minutes: string; location: string; total_marks: string; passing_marks: string; instructions: string; attachments: { name: string; size: number; type: string }[] }>({ course_id: '', lecturer_id: '', title: '', test_type: 'quiz', test_date: '', duration_minutes: '60', location: '', total_marks: '50', passing_marks: '25', instructions: '', attachments: [] });

  const { data: courses } = useQuery({ queryKey: ['courses'], queryFn: async () => { const r = await fetch('/api/courses'); return r.json(); } });
  const { data: lecturers } = useQuery({ queryKey: ['lecturers'], queryFn: async () => { const r = await fetch('/api/users?role=lecturer'); return r.json(); } });

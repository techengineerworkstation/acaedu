...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'lecture_videos') THEN
    RAISE NOTICE 'Table lecture_videos does not exist, skipping...';
  ELSE
    IF NOT EXISTS (SELECT 1 FROM pg_policy WHERE tablename = 'lecture_videos' AND policyname = 'Authenticated can view videos') THEN
